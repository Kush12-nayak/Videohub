...
import vid1 from "../Video/GendaPhool.mp4"
...
const Video = () => {
    ...
    const [videosrc,setvidosrc]=useState(VideoArr[0])

  return (
    <Stack direction={["column","row"]} h={'100vh'}>
        <VStack w={'full'}>
            <video src={videosrc} controls controlsList='nodownload' className='video'
            style={{
                width: '100%',
            }}>
            </video>
        ...
        <VStack w={['full','xl']} overflowY={'auto'} p='8' spacing={"8"}
         alignItems={"stretch"}>
            {
               VideoArr.map((item,index)=>(
             <Button variant={'ghost'} colorScheme={'purple'}
             onClick={()=>(setvidosrc(item))}>Lecture {index+1}</Button>
            ))
            }
        </VStack>  
    </Stack>
